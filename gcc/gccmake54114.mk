...
COMMONFLAGS += \
    -DCORE_M4 \
    -DLPCLIB_FAMILY=LPCLIB_FAMILY_LPC5411X \
...
PDL_LIBRARY_SRCS = \
    startup_LPC5411x.c system_LPC5411x.c \
    \
    $(RTOS_SRCS) \
    \
...
$(OBJDIR)/$(PDL_LIBRARY).a : $(MAKEFILE) $(PDL_LIBRARY_C_OBJ)
	@echo "Creating library $@..."
	@$(AR) r $@ $(PDL_LIBRARY_C_OBJ)

$(OBJDIR)/$(PROJECT).hex : $(MAKEFILE) $(OBJDIR)/$(PROJECT).axf
	@$(OBJCOPY) $(OBJDIR)/$(PROJECT).axf -O ihex $@

$(OBJDIR)/$(PROJECT).axf : $(MAKEFILE) $(C_OBJ) $(CPP_OBJ) $(OBJDIR)/$(PDL_LIBRARY).a $(LINKER_SCRIPT) $(OBJDIR)/$(PDL_LIBRARY).a
	@echo "Linking into $@..."
